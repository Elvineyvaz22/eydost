...
export async function saveTaxiSession(
  ...
  const res = await fetch('/api/taxi-session', {
    ...
    body: JSON.stringify({
      ...
      pickup_lat: draft.pickupCoords?.lat ?? null,
      pickup_lng: draft.pickupCoords?.lng ?? null,
      dropoff_lat: draft.dropoffCoords?.lat ?? null,
      dropoff_lng: draft.dropoffCoords?.lng ?? null,
