...
- **Vercel Speed Insights:** `@vercel/speed-insights` via `VercelSpeedInsights.tsx` (always on in production; not cookie-gated). Passes React Router `pathname` as `route`. Vitals send on tab blur — browse 2+ pages on the live site to test. Enable in dashboard → **Speed Insights** → Enable. Requires Pro plan or add-on (~$10/mo).